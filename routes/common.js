...
router.get('/collect', function(req, res, next) {
	...
	if (req.query.type === '0') { //不喜欢
		Promise.all([new Promise(function(resolve, reject) {
			House.findOneAndUpdate({_id: req.query.id}, {$inc: {collects: -1}}, function(err, data) {
					if (err) {
						reject(err);
					} else {
						resolve();
				...
				User.findOneAndUpdate({_id: req.session.userId}, {$pull: {collect: req.query.id}}, function(err, data) {
					if (err) {
						reject(err);
					} else {
						resolve();
		...
		Promise.all([new Promise(function(resolve, reject) {
			House.findOneAndUpdate({_id: req.query.id}, {$inc: {collects: 1}},{new: true}, function(err, data) {
					if (err) {
						reject(err);
					} else {
						resolve();
				...
				User.findOneAndUpdate({_id: req.session.userId}, {$addToSet: {collect: req.query.id}}, function(err, data) {
					if (err) {
						reject(err);
					} else {
						resolve();
